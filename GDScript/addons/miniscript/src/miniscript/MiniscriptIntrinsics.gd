...
class_name Intrinsic
    ...
    ## </summary>
    public static Intrinsic GetByID(i:int d):
        return all[id]
    ...
    func AddParam(string name, Value defaultValue = null):
    ...
    func AddParam(string name, double defaultValue):
        Value defVal
        if (defaultValue == 0):
    ...
    ## <param name="name">parameter name</param>
    ## <param name="defaultValue">default value for this parameter</param>
    func AddParam(string name, string defaultValue):
        Value defVal
        if (string.IsNullOrEmpty(defaultValue)):
            defVal = ValString.empty
        elif (defaultValue == "__isa"):
            defVal = ValString.magicIsA
        elif (defaultValue == "self"):
            defVal = _self
    ...
    ## </summary>
    public static Result Execute(i:int d, TAC.Context context, Result partialResult):
    ...
    ## </summary>
    class_name Result:
        var done:bool       # true if our work is complete false if we need to Continue
...
public static class Intrinsics
{
    ...
    func InitIfNeeded():
        if (initialized):
            return    # our work is already done bail out.
        initialized = true
        Intrinsic f
        ...
        f.code = (context, partialResult) =>:
            ...
            if (x == 1.0):
                return new Intrinsic.Result(Math.Atan(y))
        ...
        f.code = (context, partialResult) =>:
            ...
            int codepoint = 0
            if (self != null):
                codepoint = char.ConvertToUtf32(self.ToString(), 0)
        ...
        f.code = (context, partialResult) =>:
            ...
            if (self is ValMap):
                ...
                for (i:int = 0; i < keys.Count; i++):
                    if (keys[i] is ValNull):
                        keys[i] = null
            ...
            elif (self is ValList):
                List<Value> list = ((ValList)self).values
                List<Value> indexes = new List<Value>(list.Count)
                for (i:int = 0; i < list.Count; i++):
                    indexes.Add(TAC.Num(i))
                return new Intrinsic.Result(new ValList(indexes))
            return Intrinsic.Result.Null
        ...
        f.code = (context, partialResult) =>:
            ...
            if (self is ValList):
                ...
                int idx
                if (after == null):
                    idx = list.FindIndex(x => x == null ? value == null : x.Equality(value):
                    == 1):
                else:
                    int afterIdx = after.IntValue()
                    if (afterIdx < -1):
                        afterIdx += list.Count
                    if (afterIdx < -1 || afterIdx >= list.Count - 1):
                        return Intrinsic.Result.Null
                    idx = list.FindIndex(afterIdx + 1, x => x == null ? value == null : x.Equality(value) == 1):
                if (idx >= 0):
                    return new Intrinsic.Result(idx)
            elif (self is ValString):
                string str = ((ValString)self).value
                if (value == null):
                    return Intrinsic.Result.Null
                ...
                foreach (var kv in map.map):
                    if (!sawAfter):
                        if (kv.Key.Equality(after):
                            == 1):
                            sawAfter = true
                    else:
                        if (kv.Value == null ? value == null : kv.Value.Equality(value):
                            == 1):
                            return new Intrinsic.Result(kv.Key)
            return Intrinsic.Result.Null
        ...
        f.code = (context, partialResult) =>:
            ...
            Value value = context.GetLocal("value")
            if (index == null):
                throw new RuntimeException("insert: index argument required")
            if (!(index is ValNumber)):
            ...
            if (self is ValList):
                List<Value> list = ((ValList)self).values
                if (idx < 0):
                    idx += list.Count + 1 # +1 because we are inserting AND counting from the end.
        ...
        f.code = (context, partialResult) =>:
            ...
            foreach (var intrinsic in Intrinsic.all):
                if (intrinsic == null || string.IsNullOrEmpty(intrinsic.name)):
                    continue
        ...
        f.code = (context, partialResult) =>:
            ...
            for (i:int = 0; i < src.values;.Count i++):
        ...
        f.code = (context, partialResult) =>:
            Value self = context.self
            if (self == null):
                throw new RuntimeException("argument to 'replace' must not be null")
            ...
            if (self is ValMap):
                ...
                foreach (Value k in selfMap.map.Keys):
                    if (selfMap.map[k].Equality(oldval):
                        == 1):
                        ...
                        if (maxCount > 0 && count == maxCount):
                            break
                if (keysToChange != null):
                    foreach (Value k in keysToChange):
                ...
                if (string.IsNullOrEmpty(oldstr)):
                    throw new RuntimeException("replace: oldval argument is empty")
        ...
        f.code = (context, partialResult) =>:
            ...
            if (seq is ValList):
                ...
                if (toIdx > str.Length):
                    toIdx = str.Length
                if (toIdx - fromIdx <= 0):
                    return Intrinsic.Result.EmptyString
        ...
        f.code = (context, partialResult) =>:
            ...
            if (list == null || list.values.Count < 2):
                return new Intrinsic.Result(self)

            IComparer<Value> sorter
            if (context.GetVar("ascending").BoolValue()):
            ...
            if (byKey == null):
                ...
                for (i:int = 0; i < count i;++):
                    ...
                    elif (item is ValList):
                        ValList itemList = (ValList)item
                        if (byKeyInt > -itemList.values.Count && byKeyInt < itemList.values.Count):
                            arr[i].sortKey = itemList.values[byKeyInt]
        ...
        f.code = (context, partialResult) =>:
            ...
            while (pos < self.Length):
                int nextPos
                if (maxCount >= 0 && result.values.Count == maxCount - 1):
                    nextPos = self.Length
                elif (delim.Length == 0):
                    nextPos = pos + 1
                ...
                if (pos == self.Length && delim.Length > 0):
        ...
        f.code = (context, partialResult) =>:
            ...
            if (x == null):
                return new Intrinsic.Result(ValString.empty)
    ...
    func CompileSlice(List<TAC.Line> code, Value list, Value fromIdx, Value toIdx, int resultTempNum):
